...
- [Git Command](#git-command)
  ...
  - [Git常见场景运用：](#git常见场景运用)
    ...
    - [你想在当前git仓库回退到上一个版本，且不丢失现在的更改，应该怎么做呢？](#你想在当前git仓库回退到上一个版本且不丢失现在的更改应该怎么做呢)
      ...
      - [使用新的分支:](#使用新的分支)
  - [为什么你把要忽略的内容写入了gitignore，git push时还是把gitignore中的内容上传了？是有什么操作顺序吗？](#为什么你把要忽略的内容写入了gitignoregit-push时还是把gitignore中的内容上传了是有什么操作顺序吗)
    - [原因：](#原因)
    - [解决方案：](#解决方案)
...
🚨🚨🚨**注意：执行这些步骤将会删除Git历史记录和版本控制信息，所以请在操作之前确保您不再需要这些信息。**<br>
