...
### 将txt文件中的内容读取，存储为集合元素：

假设你的`words.txt`文件格式如下:<br>
...
要将这个文件读取并组成一个集合，你可以使用以下Python代码：
...
🤭解释: 这段代码不会报错的原因在于循环的起始条件。<br>

代码中，`data` 是一个空列表，因此 `for i in data:` 这个循环实际上根本不会开始执行。<br>

因为列表是空的，所以循环体内的代码（即 `res = i['ner']` 和 `print(res)`）不会被执行。<br>

**‼️‼️‼️如果列表不为空，并且其中的元素不是包含 `'ner'` 键的字典，那么这段代码就会报错。** 但在这个特定的情况下，由于循环从未开始，因此不会出现错误。<br>
...
# 单个值的例子
single_value = 'hello'
print(ensure_list(single_value))  # 输出: ['hello']

# 已经是列表的例子
list_value = ['hello', 'world']
print(ensure_list(list_value))  # 输出: ['hello', 'world']

# 数字的例子
number = 42
print(ensure_list(number))  # 输出: [42]

# 空值的例子
none_value = None
print(ensure_list(none_value))  # 输出: [None]
```
...
包含 `if __name__ == '__main__':` 代码的执行顺序:<br>

![Alt text](./python_basic_grammar_materials/if_name的执行顺序.png)
