...
> 你可能见过 `db_host = os.getenv('DB_HOST')` 写为 `db_host = os.getenv('DB_HOST', '默认值')`。

> 在Python中，`os.getenv`函数用于从环境变量中获取一个值。这个函数接受两个参数：
> 1. 第一个参数是环境变量的名称，即您想要获取的变量的名字。
> 2. 第二个参数是可选的，默认值，用于在指定的环境变量不存在时返回。

> 使用默认值是一种保护措施，确保即使环境变量没有正确设置，程序也不会因为缺少必要的配置而立即崩溃。

> 不过，在实际应用中，您可能不想在代码中提供敏感信息的默认值。相反，如果环境变量未设置，您可能更希望程序抛出错误或以其他方式通知您，这样您就可以确保所有配置都是按预期设置的。
