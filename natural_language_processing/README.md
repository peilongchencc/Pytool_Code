...
明明代码已经指定了只使用"GPU0":<br>

```python
device_map = "cuda:0" if torch.cuda.is_available() else "auto"
# "cuda:0" 就是第一张卡，"cuda:1" 就是第二张卡，同时指定多卡用 "cuda:[0,1,2]"
```
...
```txt
Every 2.0s: nvidia-smi                                                                                                                                                                      5f7d2ce68c26: Thu Mar  7 17:00:49 2024

Thu Mar  7 17:00:49 2024
+-----------------------------------------------------------------------------+
...
| N/A   29C    P0    36W / 250W |   9296MiB / 40960MiB |      0%      Default |
|                               |                      |             Disabled |
+-------------------------------+----------------------+----------------------+
|   1  NVIDIA A100-PCI...  On   | 00000000:00:08.0 Off |                    0 |
| N/A   27C    P0    31W / 250W |      2MiB / 40960MiB |      0%      Default |
|                               |                      |             Disabled |
+-------------------------------+----------------------+----------------------+
|   2  NVIDIA A100-PCI...  On   | 00000000:00:09.0 Off |                  Off |
| N/A   28C    P0    32W / 250W |      2MiB / 40960MiB |      0%      Default |
|                               |                      |             Disabled |
+-------------------------------+----------------------+----------------------+
|   3  NVIDIA A100-PCI...  On   | 00000000:00:0A.0 Off |                    0 |
| N/A   28C    P0    34W / 250W |      2MiB / 40960MiB |      0%      Default |
|                               |                      |             Disabled |
+-------------------------------+----------------------+----------------------+
...
你的代码确实指定了模型只使用"GPU0"进行运算，这是通过设置`device_map`为`"cuda:0"`实现的，如果检测到CUDA环境可用。这意味着模型和所有相关的张量操作将被指定运行在第一块GPU上。<br>

至于其他GPU显示2MiB内存使用的情况，这并不是因为你的模型或代码直接使用了这些GPU。在CUDA环境下，即使没有进行计算任务，一些GPU资源也可能被框架（如PyTorch）或系统进程占用。<br>

🔥🔥🔥这种情况通常发生在CUDA环境初始化时，NVIDIA的CUDA驱动会为每个检测到的GPU分配一小部分内存，用于管理和维护GPU的状态，**这是正常现象** 。<br>

这2MiB的内存占用可能是CUDA驱动或运行时在进行设备枚举、状态监控或者是为某些后台任务保留的资源。这种微小的内存占用对于大多数应用来说可以忽略不计，不会影响GPU的主要计算任务。<br>
